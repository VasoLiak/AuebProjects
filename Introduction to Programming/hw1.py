#θεματα παρακατω ασκησεων: 1)Epistrefei tin timi 1/(2 + 3/(x+4) + 1/x)
#                          2)Epistrefei timi isi me sqrt(add(pow(add(x, y), 2), pow(sub(x, y), 2))) 
#                          3)>>> how()...1 2 3
#                          4)>>> func0('I') + ' ' + func0('Bi') + ' ' + 'Spider'...'Itsy Bitsy Spider'
#                          5)>>> func1(1, '*', '-')...'*'
#                          6)>>> func2('la')...'la-la'
#                          7)Ypologizei to a8roisma twn (dekadikwn) pshfiwn tou x
#                          8)Ypologizei to a8roisma twn pshfiwn synexws ews otou to apotelesma exei ena mono pshfio
#                          9)Emfanizei ton xarakthra + 'h - x fores
#                          10)Ypologizei to mikrotero pshfio tou x
#                          11)Ypologizei to plh8os twn emfanisewn enos pshfiou
...
def func1(n, c, s):
    ...
    return (n-1)*(c if n==1 else (n*c + s))+ n*c
